export const getPageList = async () => {
    const res = await fetch(`${import.meta.env.PAYLOAD_URL}/api/pages/list`);
    if(!res.ok){
...
export const getPageBySlug = async (slug: string) => {
    const res = await fetch(`${import.meta.env.PAYLOAD_URL}/api/pages?depth=1&where[slug][equals]=${slug}`);
    if(!res.ok){
...
export const getAPIColors = async () => {
    const res = await fetch(`${import.meta.env.PAYLOAD_URL}/api/theme_colors`);
    if(!res.ok){
        throw new Error("Failed to fetch page data.");
    }
    const data = await res.json();
